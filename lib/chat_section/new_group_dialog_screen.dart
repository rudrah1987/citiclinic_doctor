  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Text(
            'New Group',
            style: TextStyle(color: _darkTheme ? Colors.white : Colors.black, fontWeight: FontWeight.bold),
          ),
          centerTitle: true,
        ),
  ...
  _buildDialogOccupants() {
    ...
    _getOccupants() {
      return ListView.builder(
        ...
        itemBuilder: users!=null?_getListItemTile:Container(),
      );
    }
    ...
    if (_cubeDialog.name == null || _cubeDialog.name.length < 5) {
      ...
      createDialog(_cubeDialog).then((createdDialog) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => ChatDialogScreen(currentUser, createdDialog,false,""),
          ),
        );
      }).catchError(_processDialogError);
    }
